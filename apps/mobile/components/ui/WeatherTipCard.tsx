  ...
  if (state.kind === "denied" || state.kind === "error") {
    return (
      <TouchableOpacity style={cardStyle} onPress={() => { setState({ kind: "loading" }); load(); }} activeOpacity={0.8}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: "#fff", fontWeight: "600", fontSize: 14 }}>Conseil du jour</Text>
  ...
  return (
    <Animated.View entering={FadeIn} style={[cardStyle, { flexDirection: "column", alignItems: "stretch" }]}>
      <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10 }}>
        <Text style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, textTransform: "uppercase", letterSpacing: 1, flex: 1 }}>
          Conseil du jour
        </Text>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
          <Text style={{ color: "rgba(255,255,255,0.7)", fontSize: 12 }}>🌡️ {data.temp}°</Text>
          <Text style={{ color: "rgba(255,255,255,0.7)", fontSize: 12 }}>💧 {data.humidity}%</Text>
      ...
      <View style={{ gap: 8 }}>
        {advice.tips.map((tip, i) => (
          <Text key={i} style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, lineHeight: 19 }}>{tip}</Text>
        ))}
      </View>
    </Animated.View>
  );
