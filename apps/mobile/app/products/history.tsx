...
export default function ProductHistoryScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.bg }}>
      {/* Header */}
      <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingTop: 8, paddingBottom: 12, borderBottomWidth: 0.5, borderBottomColor: c.borderLight }}>
        ...
        <Text style={{ color: c.text, fontWeight: "700", fontSize: 18, flex: 1 }}>Produits scannés</Text>
        <Text style={{ color: c.textFaint, fontSize: 12 }}>{found.length}</Text>
      </View>
      ...
      {isLoading ? (
        ...
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 32 }}>
          ...
          <TouchableOpacity onPress={() => refetch()}>
            <Text style={{ color: TERRACOTTA, fontWeight: "600" }}>Réessayer</Text>
          </TouchableOpacity>
        </View>
      ) : found.length === 0 ? (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 32 }}>
          <Text style={{ fontSize: 44, marginBottom: 12 }}>🧴</Text>
          <Text style={{ color: c.text, fontSize: 16, fontWeight: "600", marginBottom: 6 }}>Aucun produit scanné</Text>
          ...
          <TouchableOpacity onPress={() => router.replace("/products/scan")} style={{ backgroundColor: TERRACOTTA, borderRadius: 14, paddingHorizontal: 24, paddingVertical: 12 }}>
            <Text style={{ color: "#fff", fontWeight: "700" }}>📷 Scanner un produit</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ padding: 16, gap: 10 }} showsVerticalScrollIndicator={false}>
            ...
            return (
              <Animated.View key={p.id} entering={FadeInDown.delay(i * 40)}>
                <View style={{ backgroundColor: c.bgCard, borderWidth: 0.5, borderColor: c.borderLight, borderRadius: 16, padding: 14, flexDirection: "row", gap: 12, alignItems: "center" }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text numberOfLines={1} style={{ color: c.text, fontWeight: "600", fontSize: 14 }}>
                      {p.product_name || "Produit"}
                    </Text>
                    {p.brand ? (
                      <Text numberOfLines={1} style={{ color: c.textMuted, fontSize: 12, marginTop: 1 }}>{p.brand}</Text>
                    ) : null}
          ...
          <TouchableOpacity onPress={() => router.replace("/products/scan")} style={{ marginTop: 8, borderWidth: 0.5, borderColor: c.border, borderRadius: 14, paddingVertical: 14, alignItems: "center" }}>
            <Text style={{ color: TERRACOTTA, fontWeight: "600", fontSize: 14 }}>📷 Scanner un nouveau produit</Text>
          </TouchableOpacity>
        </ScrollView>
      )}
    </SafeAreaView>
