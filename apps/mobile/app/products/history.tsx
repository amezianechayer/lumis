...
export default function ProductHistoryScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#0A0A0A" }}>
      {/* Header */}
      <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingTop: 8, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.08)" }}>
      ...
      {isLoading ? (
        ...
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 32 }}>
          ...
          <TouchableOpacity onPress={() => refetch()}>
            <Text style={{ color: "#C9A84C", fontWeight: "600" }}>Réessayer</Text>
          </TouchableOpacity>
        </View>
      ) : found.length === 0 ? (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 32 }}>
          ...
          <TouchableOpacity onPress={() => router.replace("/products/scan")} style={{ backgroundColor: "#C9A84C", borderRadius: 14, paddingHorizontal: 24, paddingVertical: 12 }}>
            <Text style={{ color: "#0A0A0A", fontWeight: "700" }}>📷 Scanner un produit</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ padding: 16, gap: 10 }} showsVerticalScrollIndicator={false}>
          {found.map((p: ScannedProduct, i) => {
            ...
            return (
              <Animated.View key={p.id} entering={FadeInDown.delay(i * 40)}>
                <View style={{ backgroundColor: "rgba(255,255,255,0.04)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", borderRadius: 16, padding: 14, flexDirection: "row", gap: 12, alignItems: "center" }}>
          ...
          <TouchableOpacity onPress={() => router.replace("/products/scan")} style={{ marginTop: 8, borderWidth: 1, borderColor: "rgba(201,168,76,0.4)", borderRadius: 14, paddingVertical: 14, alignItems: "center" }}>
            <Text style={{ color: "#C9A84C", fontWeight: "600", fontSize: 14 }}>📷 Scanner un nouveau produit</Text>
          </TouchableOpacity>
        </ScrollView>
      )}
    </SafeAreaView>
